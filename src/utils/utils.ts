...
export const sortItems = (allItems: { [key: string]: any }) => {
  ...
  return Object.fromEntries(
    Object.entries(allItems).sort(([, itemA], [, itemB]) => {
      if (
        itemA.status === "not-completed" &&
        itemB.status !== "not-completed"
      ) {
        return -1;
      }
      if (
      ...
      if (
        priorityorder[itemA.priority as Priority] >
        priorityorder[itemB.priority as Priority]
      )
        return -1;
      if (
        priorityorder[itemB.priority as Priority] >
      ...
      const timeA =
        itemA.fromtime && itemA.totime
          ? `${itemA.fromtime} - ${itemA.totime}`
          : "";
      const timeB =
        itemB.fromtime && itemB.totime
          ? `${itemB.fromtime} - ${itemB.totime}`
          : "";
...
export const applyFilters = (
  ...
  if (priorityFilter !== "All") {
    filtered = Object.fromEntries(
      Object.entries(filtered).filter(
        ([_, item]) => item.priority === priorityFilter,
      ),
    );
  }
  ...
  if (tagFilter.length > 0) {
    ...
    filtered = Object.fromEntries(
      Object.entries(filtered).filter(([_, item]) =>
        tagFilter.some((selectedTag) => selectedTag.value === item.tag),
      ),
    );
  }
  ...
  if (statusFilter !== "All") {
    filtered = Object.fromEntries(
      Object.entries(filtered).filter(
        ([_, item]) => item.status === statusFilter,
      ),
    );
  }
  return filtered;
